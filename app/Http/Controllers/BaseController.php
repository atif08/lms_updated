...
class BaseController extends Controller {
    ...
    public function __construct() {
        $this->middleware(['auth', 'accessibility']);
    ...
    protected function renderView($view, $data = []) {
        return view($view, array_merge([
            ...
            'currency'        => is_null($this->current_account) ? 'USD' : $this->current_account->getCurrency(),
            'settings_page'   => $this->settings_page,
            'date_object'     => $this->date_object,
        ], $data));
    }

    protected function createForm(FormBuilder $form_builder, $form_class, array $options = [], array $data = []): Form {
        return $form_builder->create($form_class, $options, array_merge([
            'user'            => $this->user,
            'selected_user'   => $this->selected_user,
            'current_account' => $this->current_account,
