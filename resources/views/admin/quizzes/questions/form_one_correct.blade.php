                        ...
                        <div class="form-group">
                            <div class="col-lg-12 mb-2 mt-2">
                                ...
                                <div id="answers_container">
                                    <input type="hidden" id="questionOptionsCount" value="{{count(@$question->options()->get())}}">
                                    @foreach($question->options()->get() as $index => $answer)
                                        <div class="answer" data-index="{{ $index }}">
                                            <div class="row mb-1">
                                                <div class="col-md-1 selection-list d-flex justify-content-center align-items-center">
                                                    <label class="answer-radio-label" for="answer_status_{{ $index }}">
                                                        <input style="transform: scale(1.5);" type="radio" class="answer_status" id="answer_status_{{ $index }}" name="answer_status[]" value="{{ $index }}" {{ $answer->is_correct ? 'checked' : '' }}>
                                                    </label>
                                                </div>
                                                <div class="col-md-9">
                                                    <input type="text" name="answer[]" class="form-control" id="answer_{{ $index }}" value="{{ $answer->name }}" placeholder="Enter a possible answer" autocomplete="off">
                                                </div>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            let answerIndex = $('#questionOptionsCount').val(); // Start index for new answers
...
<div class="col-md-1 selection-list d-flex justify-content-center align-items-center">
                        <label class="answer-radio-label" for="answer_status_${index}">
                            <input style="transform: scale(1.5);" type="radio" class="answer_status" id="answer_status_${index}" name="answer_status[]" value="${index}">

                        </label>
                    </div>
                    <div class="col-md-9">
                        <input type="text" name="answer[]" class="form-control" id="answer_${index}" placeholder="Enter a possible answer" autocomplete="off">
                    </div>
                    <div class="col-md-2 answer-action-icons text-right">
                        <i class="move-handle fa fa-arrows" aria-hidden="true"></i>
                        <i class="delete fa fa-trash" aria-hidden="true"></i>
        ...
        `;
            ...
            // Optionally add functionality to remove answer fields (this can be added based on your needs)
